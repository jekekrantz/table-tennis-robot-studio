...
4. Tap that bounce on a local, uncertainty-sized view which expands to use the available phone height. The entire view represents measurable physical space: its centimetre grid continues over both the table and the off-table ground without a decorative gutter. The initial view includes the nearest useful intersection (endline/net × sideline/centre line), and every visible table line is named. All ruler numbers are centimetres and use clean 1/2/5/10/20/50 cm intervals as the zoom changes. Values are never hidden by the expected region; each value is placed just outside its table landmark and its landmark name is farther out on that same, ball-opposite side. Drag to translate and pinch to zoom; a wheel or trackpad also zooms on desktop. The grid sits underneath the official markings. The 2 cm edge markings and 3 mm centre line are drawn at physical scale. The top-view net is a symbolic mesh band at the exact net plane, extends 15.25 cm past each sideline, and has visible posts. The translucent ellipse shows the modeled two-dimensional 99% landing region from normal shot dispersion plus current pose uncertainty; it has no centre target and is drawn above the table geometry, while ruler values and names remain above it. The dark ground remains clickable for misses outside the table. **Fire calibration shot** is the prominent action both before a shot and when repeating one.
