(() => {
  ...
  aiDialog.innerHTML = `<div class="dialog-frame feature-dialog-frame ai-frame">
    <header><div><p class="eyebrow">AI assist</p><h2>Ask about the app or change a drill</h2><p>Get screen-aware help or describe a drill in table-tennis language.</p></div><button class="icon-button" data-close type="button">×</button></header>
    <details class="feature-details ai-context-details"><summary>Context used for this request</summary><p id="aiContextSummary" class="helper"></p><p class="helper">Screen name, active drill name/source, visible editor selection, general robot status, and current activity may be added. Drill requests also include the active drill definition. Bluetooth logs, device identifiers, API keys, calibration values, and unrelated stored data are not included.</p></details>
  ...
  function localHelp(prompt, context) {
    ...
    if(p.includes('calibrat'))return 'Open Robot from the main navigation, then choose Calibration. Guided calibration collects physical measurements; Update pose on the Run screen checks the robot position for the selected drill.';
  ...
  function currentAiContext() { return api.getAiContext?.() || {screen:'Unknown',activeDrill:null,drillSource:null,selection:null,robotStatus:null,activity:null}; }
  function contextSummary(context) { return [`Screen: ${context.screen||'Unknown'}`,context.activeDrill?`Drill: ${context.activeDrill} (${context.drillSource||'unknown source'})`:null,context.selection?`Selection: ${context.selection}`:null,context.robotStatus?`Nova: ${context.robotStatus}`:null,context.activity?`Activity: ${context.activity}`:null].filter(Boolean).join(' · '); }
  ...
  function openAiDialog() { const context=currentAiContext();aiState.proposalIntent='edit';$('aiDrillPrompt').value='';$('aiProposal').hidden=true;$('aiHelpAnswer').hidden=true;$('aiContextSummary').textContent=contextSummary(context);$('aiApplyBtn').textContent='Apply changes';$('aiStatus').textContent=api.isActiveBuiltIn()?'Ask about this screen, or ask for a change to copy the built-in drill first.':'Ask about this screen, or describe what you want to create or change.';showDialog(aiDialog); }
  function externalRequest() { const request=$('aiDrillPrompt').value.trim(); const help=promptRequestsAppHelp(request);return Core.buildExternalAiRequest({ currentDrill: help||promptRequestsFreshDrill(request)?null:api.getActiveDrill(), userRequest: request, appContext:currentAiContext(), requestKind:help?'help':'drill' }); }
  ...
  async function callByok(prompt) {
    if(!aiState.key)throw new Error('Enter an API key first.');if(!aiState.model)throw new Error('Enter the provider model ID first.');const help=promptRequestsAppHelp(prompt);const req=Core.buildExternalAiRequest({currentDrill:help||promptRequestsFreshDrill(prompt)?null:api.getActiveDrill(),userRequest:prompt,appContext:currentAiContext(),requestKind:help?'help':'drill'});
  ...
  $('aiGenerateBtn').addEventListener('click', async()=>{
    ...
    $('aiHelpAnswer').hidden=true;$('aiGenerateBtn').disabled=true;$('aiStatus').textContent='Generating…';try{
      ...
      else if(help){showAiHelp(localHelp(prompt,currentAiContext()));}
