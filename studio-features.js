(() => {
  ...
  aiDialog.innerHTML = `<div class="dialog-frame feature-dialog-frame ai-frame">
    <header><div><p class="eyebrow">AI assist</p><h2 id="aiDialogTitle">Ask about the app or change a drill</h2><p id="aiDialogSubtitle">Get screen-aware help or describe a drill in table-tennis language.</p></div><button class="icon-button" data-close type="button">×</button></header>
    <details class="feature-details ai-context-details"><summary>Context used for this request</summary><ul id="aiContextSummary" class="ai-context-list"></ul><p class="helper"><strong>General knowledge included:</strong> app navigation, drill creation and running, Nova setup, idle behavior, calibration, importing, exporting, and sharing.</p><p class="helper">Only the drill open on Run or Editor is included in a drill request. Bluetooth logs, device identifiers, API keys, calibration values, other drills, and unrelated stored data are not included.</p></details>
    <div class="ai-prompt-box"><textarea id="aiDrillPrompt" rows="5" placeholder="Ask about this screen or describe what you want."></textarea><div class="ai-prompt-actions"><button id="aiMicBtn" class="button ghost" type="button" aria-label="Speech input">🎙 Speak</button><button id="aiGenerateBtn" class="button primary" type="button">Generate</button></div><p id="aiSpeechHint" class="helper"></p></div>
    ...
    <section id="aiHelpAnswer" class="ai-help-answer" hidden><h3>Answer</h3><p id="aiHelpText"></p></section>
  ...
  function localHelp(prompt, context) {
    const p=String(prompt||'').toLowerCase();
    if(p.includes('stop')||p.includes('idle'))return 'Idle STOP safely stops robot activity but keeps Bluetooth connected, so starting again only needs the quick Nova initialization. The separate BLE timeout performs STOP first and then disconnects Bluetooth; reconnecting later requires the browser device picker. Configure both under Robot → Automatic idle actions.';
    if(p.includes('calibrat'))return 'Open Robot from the main navigation, then choose Calibration. Guided launch calibration collects physical measurements, Robot pose aligns the robot to the table, and Table and trajectory contains the table/model settings. Calibration values themselves are not sent to AI assist.';
    if(p.includes('new drill')||p.includes('create'))return 'Open Library and choose New drill. Add Shot, Serve, Random choice, Repeat/loop, or Sub-drill nodes with the + button in the editor.';
    ...
    const where=context?.screen?` You are currently on ${context.screen}.`:'';
    return `I can explain navigation, robot setup, calibration, running drills, and editing, or create and revise drills.${where} Ask a more specific question for step-by-step help.`;
  }
  ...
  function currentAiContext() { return api.getAiContext?.() || {screen:'Unknown',activeDrill:null,drillSource:null,selection:null,localContext:[],robotStatus:null,activity:null}; }
  function contextualDrill(context=currentAiContext()) { return context.activeDrill ? api.getActiveDrill() : null; }
  function contextSummary(context) { return [`Screen: ${context.screen||'Unknown'}`,...(Array.isArray(context.localContext)?context.localContext:[]),context.activeDrill?`Open drill: ${context.activeDrill} (${context.drillSource||'unknown source'})`:null,context.selection?`Visible selection: ${context.selection}`:null,context.robotStatus?`Nova: ${context.robotStatus}`:null,context.activity?`Activity: ${context.activity}`:null].filter(Boolean); }
  function renderContextSummary(context) { const list=$('aiContextSummary');list.replaceChildren(...contextSummary(context).map(item=>h('li',{text:item}))); }
  function aiScreenCopy(context) {
    if(context.screen==='Drill library')return {title:'Ask about drills or the library',subtitle:'Get help finding, organizing, importing, or creating drills.',placeholder:'How do I find a drill, organize my library, or create something new?',status:'Ask about this library view, or describe a new drill to create.'};
    if(context.screen==='Run drill')return {title:'Ask about running this drill',subtitle:'Get help with playback, robot setup, or adjustments to the open drill.',placeholder:'How can I run this drill, or make it more match-like?',status:api.isActiveBuiltIn()?'Ask about this run screen, or change the open drill by creating an editable copy.':'Ask about this run screen, or describe a change to the open drill.'};
    if(context.screen==='Drill editor')return {title:'Ask about editing this drill',subtitle:'Get help with the visible selection or change the open drill.',placeholder:'Make this more match-like and occasionally surprise me with a wide forehand.',status:api.isActiveBuiltIn()?'Ask about this editor, or change the drill by creating an editable copy.':'Ask about this editor or describe a change to the open drill.'};
    if(context.screen==='Robot')return {title:'Ask about Nova setup',subtitle:'Get help with connecting, status, diagnostics, calibration, or idle behavior.',placeholder:'What is the difference between idle STOP and disconnecting Bluetooth?',status:'Ask about the visible robot state or any Nova setup feature.'};
    if(context.screen==='Calibration')return {title:'Ask about calibration',subtitle:'Get help with the open calibration section or the overall calibration workflow.',placeholder:'What should I do in this calibration section?',status:'Ask about the visible calibration section or the overall workflow.'};
    return {title:'Ask about Table Tennis Robot Studio',subtitle:'Get help with the app or create a drill.',placeholder:'What would you like help with?',status:'Ask about the app or describe a new drill.'};
  }
  function showAiHelp(text) { $('aiProposal').hidden=true;$('aiHelpText').textContent=String(text||'No answer was returned.');$('aiHelpAnswer').hidden=false;$('aiStatus').textContent='Screen-aware app help.'; }
  function openAiDialog() { const context=currentAiContext();const copy=aiScreenCopy(context);aiState.proposalIntent=contextualDrill(context)?'edit':'create';$('aiDrillPrompt').value='';$('aiDrillPrompt').placeholder=copy.placeholder;$('aiDialogTitle').textContent=copy.title;$('aiDialogSubtitle').textContent=copy.subtitle;$('aiProposal').hidden=true;$('aiHelpAnswer').hidden=true;renderContextSummary(context);$('aiApplyBtn').textContent=aiState.proposalIntent==='create'?'Create drill':'Apply changes';$('aiStatus').textContent=copy.status;showDialog(aiDialog); }
  function externalRequest() { const request=$('aiDrillPrompt').value.trim();const context=currentAiContext();const help=promptRequestsAppHelp(request);return Core.buildExternalAiRequest({ currentDrill:help||promptRequestsFreshDrill(request)?null:contextualDrill(context), userRequest:request, appContext:context, requestKind:help?'help':'drill' }); }
  ...
  async function callByok(prompt) {
    if(!aiState.key)throw new Error('Enter an API key first.');if(!aiState.model)throw new Error('Enter the provider model ID first.');const context=currentAiContext();const help=promptRequestsAppHelp(prompt);const req=Core.buildExternalAiRequest({currentDrill:help||promptRequestsFreshDrill(prompt)?null:contextualDrill(context),userRequest:prompt,appContext:context,requestKind:help?'help':'drill'});
  ...
  $('aiGenerateBtn').addEventListener('click', async()=>{
    ...
    $('aiHelpAnswer').hidden=true;$('aiGenerateBtn').disabled=true;$('aiStatus').textContent='Generating…';try{
      const help=promptRequestsAppHelp(prompt);
      const context=currentAiContext();const current=contextualDrill(context);const intent=promptRequestsFreshDrill(prompt)?'create':(current?'edit':'create');
      if(aiState.mode==='byok'){const text=await callByok(prompt);if(help)showAiHelp(text);else showAiProposal(Core.parseExternalAiResult(text),'Proposal returned by your selected AI provider.',{intent});}
      else if(help){showAiHelp(localHelp(prompt,context));}
      else {const result=localGenerate(prompt,current);showAiProposal(proposalPortableFromDrill(result.drill),result.summary,{intent});}
  ...
  $('aiParsePasteBtn').addEventListener('click',()=>{try{const p=Core.parseExternalAiResult($('aiPasteArea').value);closeDialog(aiPasteDialog);showAiProposal(p,'Imported from external AI response.',{intent:promptRequestsFreshDrill($('aiDrillPrompt').value.trim())?'create':(contextualDrill()?'edit':'create')});}catch(e){api.toast(e.message);}});
  $('aiImportResultBtn').addEventListener('click',()=>aiImportInput.click()); aiImportInput.addEventListener('change',async()=>{const f=aiImportInput.files?.[0];aiImportInput.value='';if(!f)return;try{showAiProposal(JSON.parse(await f.text()),'Imported AI drill file.',{intent:promptRequestsFreshDrill($('aiDrillPrompt').value.trim())?'create':(contextualDrill()?'edit':'create')});}catch(e){api.toast(e.message);}});
  ...
  function installAiButton(){const globalButton=$('aiGlobalBtn');if(globalButton)globalButton.addEventListener('click',openAiDialog);const header=document.querySelector('.editor-screen-header');if(header&&!$('aiDrillBtn'))header.insertBefore(h('button',{id:'aiDrillBtn',class:'button compact ghost',type:'button',text:'✨ AI assist','aria-label':'AI assist',onclick:openAiDialog}),$('editorShareBtn')||$('editorRunBtn'));const mount=$('inspectorAiMount');if(mount&&!$('inspectorAiBtn'))mount.appendChild(h('button',{id:'inspectorAiBtn',class:'button compact ghost',type:'button',text:'✨',title:'AI assist','aria-label':'AI assist',onclick:openAiDialog}));}
