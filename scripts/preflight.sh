#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/.."

echo "[1/5] JavaScript syntax"
node --check app.js
node --check pongbot-protocol.js
node --check pongbot-ble.js
node --check selftest.js

echo "[2/5] Protocol/BLE mock self-test"
node selftest.js

echo "[3/5] Python/shell syntax"
python3 -m py_compile scripts/serve.py
rm -rf scripts/__pycache__
bash -n scripts/serve_android_via_adb.sh

echo "[4/5] Public-tree hygiene"
if find . -type f \( -name '.env' -o -name '*.pem' -o -name '*.key' -o -name '*.p12' -o -name '*.pfx' -o -name '*.pcap' -o -name '*.pcapng' -o -name '*.har' \) -print -quit | grep -q .; then
  echo "Potentially sensitive local file found:" >&2
  find . -type f \( -name '.env' -o -name '*.pem' -o -name '*.key' -o -name '*.p12' -o -name '*.pfx' -o -name '*.pcap' -o -name '*.pcapng' -o -name '*.har' \) -print >&2
  exit 1
fi

# Catch common credential formats. The words 'token', 'auth', and 'salt' are not
# rejected because they are legitimate protocol/code concepts in this app.
if grep -RInE --exclude-dir=.git --exclude='preflight.sh' \
  '(-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|github_pat_[A-Za-z0-9_]{20,}|gh[pousr]_[A-Za-z0-9]{30,}|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{30,}|xox[baprs]-[0-9A-Za-z-]{20,})' .; then
  echo "Potential credential pattern found; inspect before publishing." >&2
  exit 1
fi

echo "[5/5] Device-specific data check"
# Nova serials observed during development use an uppercase letter followed by
# 11 digits. Synthetic fixtures use TEST... and do not match this pattern.
if grep -RInE --exclude-dir=.git --exclude='preflight.sh' '\b[A-Z][0-9]{11}\b' .; then
  echo "Possible device-specific serial found; redact it before publishing." >&2
  exit 1
fi

echo "Preflight PASS"
